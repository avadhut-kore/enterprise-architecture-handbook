# Security Pattern: Just-in-Time (JIT) Privileged Access Pattern
...
- **Threats Mitigated**: Standing admin credentials

## 4. Architectural Solution
```mermaid
flowchart LR
    A["Untrusted / Ingress"] --> B["Security Control: Just-in-Time (JIT) Privileged Access Pattern"]
    B --> C["Protected Core Resource"]
```
Time-bound peer-approved role elevation
