# Modular Monolith: Database Boundaries & Schema Segregation

## 1. Problem & Architectural Context
Enforcing logical separation inside a single relational database by assigning a distinct PostgreSQL/SQL Server schema to each module (e.g., `ordering.*`, `billing.*`).
