...
- Salesforce event buses retain events for 72 hours. If a downstream consumer goes offline, it resumes without data loss by supplying the last received `replayId`.
- Failed record updates from Bulk API jobs must be logged to a dead-letter repository for review.
