# Legacy SAP ECC 6.0 Integration Challenges

## 1. Technical Constraints & Coexistence
- Legacy ECC instances rely on synchronous BAPIs and transactional RFC (tRFC), which lack native cloud resiliency.
- Database locking during high-concurrency external writes creates application freezes.
- Solution architects must isolate ECC behind an enterprise event bridge or iPaaS to buffer traffic and transform modern JSON into legacy IDocs.
