# Socket Management and Interface Engine Topology

## 1. Dedicated TCP Ports
In traditional hospital networks, each clinical interface runs on a dedicated static TCP port:
- Port `5001`: Inbound ADT from Epic.
- Port `5002`: Inbound ORU lab results from Roche analyzer.
- Port `5003`: Outbound radiology orders to PACS.

Integration engines must maintain persistent TCP listeners with automatic reconnection daemons.
