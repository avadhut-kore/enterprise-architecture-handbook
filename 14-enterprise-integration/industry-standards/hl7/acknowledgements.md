# HL7 Acknowledgments: Original vs. Enhanced Mode

## 1. The MSA Segment
The receiver must reply with an acknowledgment message containing an `MSA` segment:
- `AA` (Application Accept): Message successfully parsed and stored.
- `AE` (Application Error): Message syntax valid, but business rule validation failed.
- `AR` (Application Reject): Syntax error, unparseable segment, or unknown message type.
