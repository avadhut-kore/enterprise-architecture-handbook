# SMART on FHIR Security Specifications

## 1. OAuth 2.0 with PKCE
Public clients (mobile apps, single-page web apps) connecting to FHIR servers must use the OAuth 2.0 Authorization Code Flow with **PKCE (Proof Key for Code Exchange)**. Client secrets must never be embedded in frontend distributions.
