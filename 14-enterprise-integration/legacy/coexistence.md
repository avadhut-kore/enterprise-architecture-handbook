# Multi-Year Coexistence and Dual-Run Operations

## 1. Dual-Write vs. Asynchronous Replication
During multi-year migrations, legacy and modern systems must run concurrently:
- Never execute synchronous dual-writes from application code (creates distributed split-brain).
- Designate one system as the authoritative writer per bounded context and replicate asynchronously to the other.
