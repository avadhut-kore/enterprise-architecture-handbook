...
## 1. Eliminating Downstream Callbacks
Unlike thin notifications, ECST carries the entire modified entity state within the event payload:
- **Pros**: Consumers build their own local read models and never need to call the producer service back, maximizing availability.
- **Cons**: Increased event payload size; data consistency risks if consumers miss events.
