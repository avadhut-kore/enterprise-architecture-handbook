...
## 1. Synchronous vs. Asynchronous Request-Reply
- **Synchronous (HTTP / gRPC)**: Client thread blocks waiting for downstream response. Simple, but tightly couples availability.
- **Asynchronous over Messaging (JMS / AMQP)**: Client publishes request to a queue with a `CorrelationID` and a `ReplyTo` temporary queue header, allowing non-blocking response correlation.
