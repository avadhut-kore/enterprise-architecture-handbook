...
## 1. Ensuring Idempotent Consumption
Consumer writes incoming message ID to an `inbox` database table within the same local transaction that updates the domain entity. If the message ID already exists, the transaction rolls back silently.
