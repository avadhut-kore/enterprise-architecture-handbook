...
## 1. Message Queue Mechanics vs. Log Streams
- **Message Queues (AMQP / JMS)**: Messages are transient. The broker tracks consumption state per message. Once consumed and acknowledged, the message is deleted from the broker.
- **Ideal For**: Task queues, worker distribution, point-to-point command dispatch, and complex routing exchanges.
