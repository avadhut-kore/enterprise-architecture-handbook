# Hibernate & ORM Persistence Modernization

## 1. Resolving the N+1 Query Problem
Legacy Hibernate mappings frequently suffer from eager fetch joins and unbounded lazy loading:
- Refactor XML-based `hbm.xml` mappings to JPA annotations.
- Replace complex native SQL with Spring Data JPA projections and query hints (`@EntityGraph`).
