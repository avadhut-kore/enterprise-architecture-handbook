# Windows Services to .NET Background Worker Services

## 1. Containerizing Background Tasks
Migrate legacy `System.ServiceProcess.ServiceBase` daemons to **.NET BackgroundService** (`Microsoft.Extensions.Hosting.Worker`):
- Run tasks as containerized worker pods scaled elastically by Kubernetes KEDA based on queue depth.
